...
function _runPolicies() {
  _runPolicies = _asyncToGenerator(function* (ctx) {
    ...
    const policies = policyConfigs.map(policy => _objectSpread({}, policy, {
    ...
    const jsonYamlArtifacts = yield Promise.all(artifacts.filter(filename => filename.match(/\.(json|yml|yaml)$/i)).map(
    /*#__PURE__*/
    function () {
...
`);
    ...
    const runningPolicies = policies.map(
    /*#__PURE__*/
    function () {
